...
describe CommentEvaluation do
  ...
  describe '.kind' do
    context 'good評価を登録しようとした時、' do
      let(:eval) { FactoryGirl.build(:good_kind_comment_eval) }
      it {
    ...
    context 'bad評価を登録しようとした時、' do
      let(:eval) { FactoryGirl.build(:bad_kind_comment_eval) }
      it {
    ...
    context 'goog/badでもない評価を登録しようとした時、' do
      let (:eval) { FactoryGirl.build(:invalid_kind_comment_eval) }
      it {
  ...
  describe '#kind_list' do
    subject { CommentEvaluation.new.kind_list }
    it { should eq([1, 2]) }
  end

  describe 'name_scope' do
    let (:good) { FactoryGirl.build(:good_kind_comment_eval) }
    let (:bad) { FactoryGirl.build(:bad_kind_comment_eval) }
    ...
    describe '#goodは、goodと評価されたものだけを含む。' do
      subject { CommentEvaluation.good }
      it {
        should include(good)
